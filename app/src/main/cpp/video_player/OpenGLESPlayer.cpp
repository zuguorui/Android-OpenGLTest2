...
void OpenGLESPlayer::updateTexImage() {
    ...
    mTexture->updateDataToTexture(frame->data, frame->width, frame->height);
//    free(pixel);
...
void OpenGLESPlayer::refresh() {
    ...
    msgQueue.push_back(RenderThreadMessage::REFRESH);
...
void OpenGLESPlayer::renderLoop() {
    ...
    while(!exitFlag)
    {
        ...
        switch (message)
        {
            case RenderThreadMessage::SET_WINDOW:
            {
                initComponents();
                break;
            }
            case RenderThreadMessage::RESET_SIZE:
            {
                mRender->resetRenderSize(0, 0, width, height);
                break;
            }
            case RenderThreadMessage::REFRESH:
            {
                if(eglCore)
                {
                    updateTexImage();
